// app/scanner.tsx — Live Camera QR Scanner Screen
...
export default function ScannerScreen() {
  ...
  const handleCapture = async () => {
    ...
    try {
      ...
      // Auto-cropping: crop the center 80% square of the sensor image
      const pw = photo.width || 1280;
      const ph = photo.height || 1280;
      const sqSize = Math.round(Math.min(pw, ph) * 0.8);
      const originX = Math.round((pw - sqSize) / 2);
      const originY = Math.round((ph - sqSize) / 2);

      const manipulated = await ImageManipulator.manipulateAsync(
        photo.uri,
        [
          { crop: { originX, originY, width: sqSize, height: sqSize } },
    ...
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      <View style={styles.cameraContainer}>
        ...
        <View style={[styles.camTopBar, { top: 12 }]}>
          <TouchableOpacity style={styles.camNavBtn} onPress={() => router.back()}>
            <Text style={styles.camNavBtnText}>← Back</Text>
          </TouchableOpacity>
          <View style={{ flex: 1 }} />
...
const styles = StyleSheet.create({
  ...
  camTitle: { fontFamily: FontFamily.heading, fontSize: 16, color: '#fff', letterSpacing: 0.5 },
